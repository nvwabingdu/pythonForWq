...
class App(tk.Tk):
    ...
    def on_generate(self):
        ...
        if self.clip_image is not None:
            # 图片尺寸处理
            w = self.clip_image.width
            h = self.clip_image.height
            ...
            # 核心修改：单元格高度 = 图片实际高度（openpyxl 行高单位匹配）
            ws.row_dimensions[row_num].height = new_h * (72 / 96)
            # 列宽保持适配
            ws.column_dimensions['L'].width = new_w * 0.75
